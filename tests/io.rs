...
build_test_binary_once!(tokio_linelen, "tests/bins");
...
#[apply(policies)]
fn test_tokio_lines_and_create(#[case] policy: IOPolicy) {
    policy.run(
        path_to_tokio_linelen(),
        concat!(
            "1\n",
            "To\n",
            "Tre\n",
...
#[apply(policies)]
fn test_tokio_read_and_write(#[case] policy: IOPolicy) {
    policy.run(
...
#[apply(policies)]
fn test_tokio_read_to_string_and_write(#[case] policy: IOPolicy) {
    policy.run(
        path_to_tokio_revchars(),
        concat!(
            "In Xanadu did Kubla Khan\n",
            "A stately pleasure-dome decree:\n",
            "Where Alph, the sacred river, ran\n",
